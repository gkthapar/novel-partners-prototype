...
import { X, Edit, Eye, Download, Send, GripVertical, FileText, ExternalLink } from 'lucide-react';
import { Button } from './ui/button';
import { ScrollArea } from './ui/scroll-area';
import { Artifact } from '@/lib/types';
import ReactMarkdown from 'react-markdown';

interface TabbedArtifactPanelProps {
  artifacts: Artifact[];
...
export function TabbedArtifactPanel({
  artifacts,
  activeArtifactId,
  onSetActive,
  onUpdate,
  onClose
}: TabbedArtifactPanelProps) {
  ...
  const handleSave = () => {
    if (activeArtifact) {
      onUpdate(activeArtifact.id, editContent);
      setIsEditing(false);
    }
  };
  ...
  const typeLabels: Record<string, string> = {
    ...
    google_doc: 'Curriculum Document'
  };

  if (!activeArtifact) return null;

  const isGoogleDoc = activeArtifact.type === 'google_doc' && activeArtifact.googleDocUrl;
  ...
  return (
    ...
    >
      ...
      {/* Tabs */}
      <div className="border-b bg-muted/30 pl-6 flex items-center overflow-x-auto">
        {artifacts.map(artifact => (
          <div
            key={artifact.id}
            className={`group flex items-center gap-2 px-4 py-2 border-r cursor-pointer transition-colors ${
          ...
          >
            ...
            <span className="text-sm truncate max-w-[150px]">{artifact.title}</span>
            <button
              onClick={(e) => {
                e.stopPropagation();
                onClose(artifact.id);
          ...
          <div className="flex-1">
            ...
            {activeArtifact.metadata && (
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                {activeArtifact.metadata.lesson && (
                ...
                {activeArtifact.metadata.adaptedFor && (
                  <span className="inline-flex items-center px-2 py-1 rounded-md bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-100 text-xs">
        ...
        <div className="flex gap-2 mt-3">
          {isGoogleDoc && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(activeArtifact.googleDocUrl, '_blank')}
            >
              <ExternalLink className="w-4 h-4 mr-1" />
              Open in Google Docs
            </Button>
          )}
          {!isGoogleDoc && activeArtifact.isEditable !== false && (
            <>
              <Button
      ...
      <ScrollArea className="flex-1">
        <div className="h-full">
          {isGoogleDoc ? (
            <iframe
              src={`https://docs.google.com/document/d/${extractDocId(activeArtifact.googleDocUrl!)}/preview`}
...
// Helper to extract Google Docs ID
